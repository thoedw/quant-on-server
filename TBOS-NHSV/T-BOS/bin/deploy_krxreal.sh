...
IS_CONTINUE="n"
echo "***********We are deploying on ${COMPANY} krx TEST server IP: ${BOS_IP}. "
echo "Please enter y[YES] to continue doing on ${COMPANY} krx TEST server?"
read IS_CONTINUE
...
ssh -t vnst@$BOS_IP << EOF
    ...
    cp -p /VNST/tuxs/$SRC_NAME /VNST/tuxs/backup/$TODAY_DATE/$SRC_NAME.$TODAY
