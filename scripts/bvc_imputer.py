...
def bvc_classify(open_: float, high: float, low: float,
    ...
    if bar_range < 1e-6:
        ...
        # Ưu tiên 1: delta_p = close - open (nếu MASVN báo open ≠ ATC settlement)
        # Ưu tiên 2: cross-bar — so ATC settlement vs giá liên tục cuối (prev_close)
        ...
        reference = None
        if abs(delta_p) >= 1e-6:
            reference = delta_p                      # within-bar direction
        elif prev_close is not None:
            reference = close - prev_close           # cross-bar: ATC vs last continuous

        if reference is None or abs(reference) < 1e-6:
            buy_half = volume // 2                   # ATO / true doji → neutral
            return buy_half, volume - buy_half
        elif reference > 0:
...
def run_bvc_imputer(
    ...
    if not force:
        # Chỉ các bar chưa có side data HOẶC data bị corrupt (overflow)
        where_parts.append(
            "( ((buy_vol IS NULL OR buy_vol=0) AND (sell_vol IS NULL OR sell_vol=0)) OR (COALESCE(buy_vol,0) + COALESCE(sell_vol,0) > volume * 3) )"
        )
    ...
    if total_to_fix == 0:
        ...
        return {'total_found': 0, 'preview': 0, 'imputed': 0, 'skipped': 0, 'elapsed_sec': 0}

    # ── Fetch ──────────────────────────────────────────────────
    # prev_close: close của nến liền trước trong TOÀN BỘ DB (không chỉ filtered bars).
    # Dùng correlated subquery thay vì LAG() vì LAG() chỉ nhìn rows trong WHERE clause:
    #   → Nếu bars 14:27-14:29 đã classified (không trong WHERE), LAG trả NULL cho bar 14:45
    #   → Correlated subquery luôn tìm đúng prev bar bất kể classification status
    rows = conn.execute(
        f"""
        SELECT sp.rowid, sp.open, sp.high, sp.low, sp.close, sp.volume,
               (SELECT prev.close
                FROM   stock_prices prev
                WHERE  prev.security_id = sp.security_id
                  AND  prev.interval    = '1m'
                  AND  prev.trade_time  < sp.trade_time
                  AND  date(prev.trade_time) = date(sp.trade_time)
                  AND  prev.volume > 0
                ORDER  BY prev.trade_time DESC
                LIMIT  1
               ) AS prev_close
    ...
    for rowid, o, h, l, c, v, prev_c in rows:
        ...
        try:
            buy_vol, sell_vol = bvc_classify(
                ...
                prev_close=float(prev_c) if prev_c is not None else None
            )
